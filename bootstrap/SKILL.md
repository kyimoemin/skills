...
1. **Read the brief.** $ARGUMENTS is the path to a /shape product brief;
   if missing or unreadable, stop and point at /shape — bootstrap builds
   foundations for a defined product, not for a guess. Unresolved open
   product decisions in the brief don't block you (they block /architect),
   but flag them.
...
6. **Commit the brief** into `docs/product/` (move it there if /shape
   wrote it elsewhere) — the durable product contract lives in the repo.
...
9. **Report:** stack/tracker/hosting decisions, repo location, what the
   ...
   ready-to-run next command: `/architect docs/product/<brief>.md`.
