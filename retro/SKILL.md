...
1. **Identify the iteration.** $1 as the sprint id if given, else the
   ...
   pick one. No `.sprint/` dir or no run logs → the sprint may have run on
   another machine: restore from the archive ref
   ...
   files on macOS's tar, GNU tar wants `--skip-old-files`) and look
   again. Still nothing → say so and stop; there is nothing to retro.
2. **Gather, per source:**
...
3. **Analyze** — numbers first, then causes:
   ...
   - Which changes indict the WORKFLOW, not the project: a pattern that
     names a skill or agent doc rather than this codebase (implementers
     repeatedly missing the same convention, reviewers flagging the same
     class across features, a dispatch slot that keeps getting asked
     about) is a defect in the suite itself, and a project ticket can't
     fix it.
